...
def generate_carousel():
    try:
        ...
        print("🎨 Обрабатываю Photo слайд ТОЛЬКО с изображением недвижимости...")
        # Для photo слайда используем ТОЛЬКО propertyimage
        photo_replacements = {}
        if 'dyno.propertyimage' in replacements:
            photo_replacements['dyno.propertyimage'] = replacements['dyno.propertyimage']
            print(f"📸 Photo слайд: только propertyimage = {replacements['dyno.propertyimage'][:50]}...")
